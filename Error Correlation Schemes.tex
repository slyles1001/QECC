...
We can use the IBM's \verb|qasm| quantum computer simulator to validate our scheme. The results are identical to the above two sections, so we omit them here. All code is available at the github link at the beginning of the appendix.
...
\vskip .5in 
{\bf \hskip 1in  Circuit diagram here.} 


\vskip -.7in 
{\hfill \includegraphics[height=1.5in,width=2in]{./img/q3qasm.png}}


One may consider using $|q_2\ra = U|0\ra$ and  $|q_1q_0\ra = V|00\ra$
for unitary $U \in M_2, V \in M_4$, and apply our scheme.
One should get perfect decoding of $|q_1q_0\ra = V|00\ra$.


\vskip 1in 

One may do simulation and get the statistics for $n = 4,5,6$, etc.

\subsection{Using IBM's quantum computer}

We can use the IBM's online quantum computer (with noise) to implement our scheme.
...
Here we illustrate our hybrid quantum error correction scheme for $n$-qubit system
when $n$ is odd. In particular, we use the IBM's online quantum computer to do the
$n = 3, 5$ cases to demonstrate the scheme including the recursion step.

Consider the correlated channel $\cE: M_8 \rightarrow M_8$ such that
$$\cE(\hat \rho) 
= p_0 \hat \rho + p_1 X_3 \hat \rho X_3^\dag 
+ p_2 Y_3 \hat \rho Y_3^\dag + p_3 Z_3 \hat \rho Z_3^\dag.$$
Let $P_3$ be constructed in Section 2.
We can do the following quantum error correction scheme using 1 
arbitrary qubit (ancilla) $\sigma\in D_2$ to protect the 2 data qubit 
$\rho \in D_4$.

\begin{enumerate}

\item
Encoding: $\rho \mapsto P_3(\sigma \otimes \rho)P_3^t = \hat \rho$
using the encoding matrix constructed in Section 2.

Note that in the vector state form the encoding is done as
$$|\psi_2 \psi_1 \psi_0\ra \mapsto P|\psi_2 \psi_1 \psi_0\ra \quad \hbox{ 
if } \quad \sigma = |\psi_2\ra \la\psi_2|, 
\ \ \rho_1 = |\psi_1\ra \la\psi_1|, 
\ \ \rho_0 = |\psi_0\ra \la\psi_0|.$$

\item  Then  
\begin{eqnarray*}
P_3^t[\cE(\hat \rho)]P_3
 &=& P_3^t \cE(\hat \rho) P_3 \\
&=& p_0 (\sigma \otimes \rho) 
+ p_1 [(X_1\sigma X_1^\dag) \otimes \rho] 
 + p_2 [(Y_1\sigma Y_1^\dag)\otimes \rho] 
 + p_3 [(Z_1\sigma Z_1^\dag)\otimes \rho] \\
&=& 
\tilde \sigma \otimes \rho.
\end{eqnarray*}

\item Recovering channel: 
Note that $P_3^t(\cE(P_3(\sigma \otimes \rho)P_3^t)P_3 =
\sigma \otimes \rho$. So, applying the map $\tilde \rho \mapsto 
\tr_1(P_3^t\tilde \rho P_3)$ will recover $\rho$.
\end{enumerate}
...
\medskip
Step 0. Prepare $\sigma \in D_2, \rho\in D_4$.


\medskip
Step 1. Apply the  encoding by $\sigma\otimes \rho\mapsto 
P_3(\sigma\otimes \rho)P_3^t$.

\medskip 
Step 2. Apply the quantum correlated channel $\cE$.


\medskip
Step 3. Apply the decoding $X \mapsto \tr_1(P_3^tXP_3)$
to get $\rho$ back.
...
In Step 0, we only need to test $\sigma, \rho = \rho_1 \otimes \rho_2$ 
with 
$$\sigma, \rho_1, \rho_2 \in \{e_1e_1^t, e_2e_2^t, (e_1e_2^t+e_2e_1^t)/2
i(e_1e_2^t-e_2e_1^t)/2 \}.$$

In Step 1. We need to use simple quantum gates (permutation matrices) to generate
the desired permutation matrix $P$.

In Step 2. 
We use the controlled not gates: $C_{12}, C_{13}, C_{21}, C_{23},
C_{31}, C_{32}$, where $C_{ij}$ is using the $i$th bit to controlled the 
$j$th bit for $i \in \{1,2,3\}$ and $j \in \{1,2,3\}-\{i\}$.
Every $C_{ij}$ is a product of 2 transpositions. In particular, we have
$$C_{12} = (5,7)(6,8), \ C_{13} = (5,6),(7,8), \ C_{21} = (3,7)(4,8),$$
$$C_{23} = (3,4)(7,8), \ C_{31} = (2,6)(4,8),  \ C_{32} = (2,4)(6,8).$$
It turns out that 
$$P = C_{21} C_{12} C_{31} C_{23} C_{12}.$$ 

Note that when we apply $A \mapsto PAP^t$ for $A \in D_8$, the corresponding
quantum circuit diagram should be $|\psi\ra \mapsto P |\psi \ra$, and the operations
$C_{rs}$ should be from left to right as $C_{12}, C_{23}, C_{31}, C_{12}, C_{21}$.
 
In Step 3,   
we can use Python program to generate the channel $\cE$.


In Step 4, we can apply the decoding operation  $A \mapsto P^tAP$.
In vector state form, $|\psi\ra \mapsto P^t |\psi\ra$.
The sequence of controlled gates will be $C_{21}, C_{12}, C_{31}, C_{23}, C_{12}$.
Then we take the partial trace by adding the $(1,1)$ and $(2,3)$ blocks.

\fi

\section{Hybrid codes for the correlated channel}

Here we illustrate our hybrid quantum error correction scheme for $n$-qubit system
when $n$ is even. In particular, we use the IBM's online quantum computer to do the
$n = 4, 6$ cases to demonstrate the scheme including the recursion step.

Let $(X_4,Y_4,Z_4) = (X^{\otimes 4},Y^{\otimes 4},Z^{\otimes 4})$
Consider the correlated  quantum channel on a four-qubit
system given by
    \[ \cE(\rho) = p_0 \rho + p_1 X_4\rho X_4^\dag +  
    p_2 Y_4\rho Y_4^\dag +  p_3 Z_4\rho Z_4^\dag, \]
where $p_0, \dots , p_3$ are probabilities summing to $1$. 




We can use this channel to ``protect'' 2 classical bit
and 2 quantum bits represented as
$\sigma \otimes \rho$, where 
$\sigma \in \{|ij\ra \la ij|: 1 \le i, j \le 2\}$
and $\rho \in D_4$. Here $|00\ra\la00|,
|10\ra\la10|,|01\ra\la01|,|11\ra\la11|$
correspond to the 2 classical bits $(00), (01), (10), (11)$.

\medskip\noindent
{\bf Pseudo code}

Step 0. Consider a 4 qubit state of the form 
$\sigma \otimes \rho$, where $\rho \in D_4$ and 
$\sigma \in \{|ij\ra \la ij|: 1 \le i, j \le 2\}$.

Step 1. Encoding:  $\sigma \otimes \rho\rightarrow 
P_4(\sigma \otimes \rho)P_4^\dag$

Step 2. Apply the correlated channel $\cE$. 

Step 3. Decoding: $B \mapsto P_4^\dag BP_4$, and we should get
$\sigma \otimes \rho$. 

Step 4. We can measure the first 
two bits to get the information for the classical bit,


\qquad and keep the remaining 2-qubit state $\rho$.

\medskip\noindent
{\bf Remarks}

In Step 0, we only need to test 
$\sigma \in\{|00\ra\la00|,
|10\ra\la10|,|01\ra\la01|,|11\ra\la11|\}$,
i.e., the vector states $|00\ra, |01\ra, |10\ra, |11\ra$
and $\rho = \rho_1 \otimes \rho_2$ with 
$$\rho_1, \rho_2 \in \{e_1e_1^t, \ e_2e_2^t, \ (e_1e_2^t+e_2e_1^t)/2,
\ i(e_1e_2^t-e_2e_1^t)/2 \}.$$

In Step 1, we can use the encoding matrix $P_4$ constructed in Section 2.

In Step 3, we use python set up the code to stimulate the channel.

In Step 4, we can apply $B \mapsto P_4^\dag BP_4$.
